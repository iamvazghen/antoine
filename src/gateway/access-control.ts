...
function pairingPath(): string {
  return (
    process.env.ANTOINE_PAIRING_PATH ??
    antoinePath('pairing', 'whatsapp.json')
  );
}
...
export function buildPairingReply(code: string, senderId: string): string {
  return [
    'Antoine access request received.',
    `Sender ID: ${senderId}`,
    `Approval code: ${code}`,
    'Ask the operator to approve this code in Antoine gateway config.',
  ].join('\n');
}
