...
function debugLog(msg: string) {
  try {
    const logDir = antoinePath('debug', 'logs');
    const logPath = antoinePath('debug', 'logs', 'gateway-outbound.log');
...
function getActive(accountId?: string): ActiveListener {
  ...
  if (!first) {
    throw new Error('No active WhatsApp listener. Run antoine gateway run.');
  }
  return first;
}
